...
class SuggestionTracker {
    static private let queue = DispatchQueue(label: "com.adadapted.suggestiontracker")
    ...
    static func suggestionPresented(searchId: String, termId: String, replacement: String) {
        let lcReplacement = convertToLowerCase(input: replacement)
        let result: (term: String, userInput: String)? = queue.sync {
            guard let term = replacements[lcReplacement], let userInput = items[term] else { return nil }
            return (term, userInput)
        }
        if let result = result {
            InterceptClient.getInstance()?.trackPresented(searchId: searchId, termId: termId, term: result.term, userInput: result.userInput)
        }
    }
    ...
    static func suggestionSelected(searchId: String, termId: String, replacement: String) {
        ...
        if let result = result {
            InterceptClient.getInstance()?.trackSelected(searchId: searchId, termId: termId, term: result.term, userInput: result.userInput)
        }
    }
